...
function TicketsPageContent() {
  ...
  const generateRandomUsers = (size: number): string[] => {
    const names = [
      "DOMinator",
      "Nand",
      "Rutu",
    ...
    return Array.from({ length: size }, (_, i) => names[i % names.length]);
  };

  const randomUsers = generateRandomUsers(data?.tickets?.length || 0);
    ...
    <div className="w-full flex flex-col">
      {isLoading ? (
        <>
          <div className="flex-w-full h-[80vh]  pt-30 flex-1 items-center justify-center px-4">
            ...
            <div className="relative z-10 flex -transalte-y-1/2 flex-col items-center gap-6 text-center">
              ...
              <Heading>Fetching tickets</Heading>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="w-[100px] truncate">ID</TableHead>
              <TableHead className="text-center">Status</TableHead>
              <TableHead className="text-center">Priority</TableHead>
              <TableHead className="text-center">Alert ID</TableHead>
          ...
          <TableBody>
            {data?.tickets?.map((ticket: Ticket, index: number) => (
              <TableRow key={ticket.id}>
                ...
                <TableCell
                  className={`text-center truncate px-2 py-1 rounded-md font-semibold ${
                    ticket.status === "OPEN"
                ...
                <TableCell className="text-center truncate">
                  {ticket.userId ? ticket.userId : randomUsers[index]}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
